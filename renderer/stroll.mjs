...
const SPEED_Y = 0.011;
// 화면 가장자리에서 남기는 여백.
// 위는 **머리 위 말풍선 몫**이다: 게 12~14px + 기호 11px + 사이 3px + **풍선의 흔들림 ±2px**
// — 흔들림 몫을 빼먹고 30으로 두었더니 맨 위에 선 대기 게의 풍선 윗머리가 잘렸다(#192).
// 아래는 **노트북 몫**이다 — 상판이 발보다 4px 앞(아래)에 놓이므로 그만큼 없으면 일하는
// 순간 노트북이 잘린다.
const EDGE_X = 10;
const EDGE_TOP = 34;
const EDGE_BOTTOM = 7;
