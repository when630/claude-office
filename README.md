...
- Hold **`Ctrl+Shift`** and you can order them about. **Drag a box** to pick crabs, **right-click**
  to send the picked ones there (they spread out rather than pile on one spot), click empty space
  to clear. Let go of the keys and the sheet goes back to letting your clicks through. One that is
