...
- **`Ctrl+Shift`를 누르고 있으면** 지휘할 수 있다. 그동안 커서는 십자로 바뀐다.
  **드래그로 상자**를 그려 고르고, **우클릭한 자리**로 보낸다 — 그 자리에 파문이 한 번 퍼지고,
  여럿이면 한 점에 겹치지 않게 흩어져 선다. 빈 곳을 톡 누르면 선택이 풀리고, 키를 떼면 다시
  클릭을 통과시킨다. 일하던 게는 노트북을 접고 갔다가 **도착해서 다시 편다**
