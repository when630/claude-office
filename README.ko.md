...
- 사무실은 **손으로 당겨 본다** — `Ctrl+휠`(트랙패드는 핀치)로 확대·축소, `스페이스+드래그`나
  가운데 버튼 드래그로 화면 이동, `Ctrl+0`으로 창 폭에 맞춘 배율로 복귀(`Ctrl+=` · `Ctrl+-`도
  한 단계씩). 커서 밑에 있던 자리가 그대로 남으므로 보던 방을 잃지 않고, 확대해도 방 배치는
  다시 접히지 않는다. 배율은 픽셀이 또렷한 정수배만 쓰고 **저장하지 않는다**
