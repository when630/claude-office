...
- **집어 들어 아무 데나 놓을 수 있다.** 커서에 매달려 다리를 늘어뜨리고, 놓으면 바닥으로 떨어진다
- **마우스를 올리면** 방과 세션 이름이 뜨고, **누르면** 큰 창이 돌아오면서 그 세션이 펼쳐진다
- 게가 아닌 자리는 **클릭이 그대로 통과한다** — 마우스에게 이 한 장은 없는 것이다
- 세션이 많아도 화면이 안 덮인다: 한 번에 **여섯 마리**까지만, 급한 순으로 나간다(대기 · 헤맴 ·
  서버 장애 · 실패 · 나머지). 더 보고 싶으면 `settings.json`의 `view.strollMax`를 올린다
