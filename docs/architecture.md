...
renderer/stroll.mjs       산책의 **움직임** — 걷기·뛰기·앉기·집어 옮기기·던지기·포탈로 나오고
                          들어가기, 일을 끝낸 자축(폴짝+기지개), 지휘(고르기·보내기·머물기),
                          그리고 놀이(발자국·잡담·술래잡기·기지개·낮잠·캔, 미니 포탈로 소환하는
                          비치볼·피크닉 — 전부 idle일 때만이고 쉬는 모습은 시간대를 탄다).
                          화면 전체를 2차원으로
...
- **지휘(Ctrl+Shift)는 창에 키를 물어보지 않는다.** 산책 창은 초점을 안 받으므로 keydown이
  ...
  지휘 중에 그리는 화살표는 **게 배율을 안 탄다**(`drawCursor`가 변환을 갈아 끼워 늘 2배로
  찍는다) — 배율 4로 쓰면 40×52px짜리 화살표가 떠서 커서가 아니라 커서 그림이 된다
- **고르기 표시는 발밑 고리 하나로 두 상태를 낸다**(`stroll-view.mjs`). 상자 안에 든 게는
  앞쪽 반달(열린 고리), 놓아서 골라진 게는 닫힌 고리다 — 몸이 뒤쪽 호를 가리므로 닫힘/열림만
  다르면 구분이 안 되어 **크기도 갈랐다**(9px / 7px). 점선으로 끊어 보려다 접었다: 이 크기에서
  각도로 자르면 좌우 끝에 점이 몰려 고리가 아니라 양옆 괄호로 보인다. 흰 고리 밑에 한 픽셀
  어두운 고리를 깔아 밝은 벽지에서도 살아남게 했다. **상자 안 판정은 매 프레임 다시 센다** —
  상자를 안 움직여도 게가 걸어서 들고 난다
- **골라지는 순간의 네모는 게마다 하나씩 물린다**(`drawPickBox`). 전체를 감싸는 네모 하나가
  조여들게 해 봤더니 "저 무리를 묶었다"까지만 말하고 누구누구인지는 말하지 않았다 —
  가운데 서 있던 못 고른 게도 같이 묶인 것으로 보인다. 켜지는 순서는 **상자를 누르기 시작한
  자리에서 멀어지는 순**이라 끈 방향대로 물린다. 귀퉁이 갈고리를 그리는 좌표는 **늘 정규화된
  것**이라야 한다(x0 < x1, y0 < y1) — 끄는 상자를 그대로 넘겼더니 오른쪽 아래에서 왼쪽 위로
  끌 때 갈고리가 죄다 바깥으로 뒤집혔다
...
- **소환 도구는 게와 같은 포탈로 오간다**(`stroll.mjs`의 `startBall`·`startGather`·`stepProps`).
  상태는 in→live→out 셋이고, 붙잡힌 게(선수·손님)는 일·명령·손이 끼어드는 순간 즉시 빠진다 —
  남은 인원이 모자라면 도구는 반납되고 공용 쿨다운(`PLAY_COOL_MS`)이 걸린다.
  지휘의 머물기(`pinPets`)가 풀리는 길은 셋뿐이다: 새 이동 명령·집어 들기·일이 들어오기
- **에러코드 악당도 같은 포탈·같은 쿨다운을 쓴다**(`startVillain`·`stepVillain`). 공격 셋
  (돌진=던지기 물리 재활용 · 글리치 빔=몸을 세 조각 클립으로 찢는다 · 버그 뿌리기)을 섞고,
  난동 `RAGE_MS` 뒤 게들이 미니 포탈로 무기를 받아 반격한다(종이 투척 → hp 1 → 망치 피니셔).
  사용자가 잡아 휙 던지면 즉시 격퇴, 살살 놓으면 다시 일어난다. **표적은 늘 쉬는 게뿐이고**
  맞은 게도 일이 들어오면 즉시 복구된다 — 악당·무기·버그는 상태 신호가 아니라 연출이다.
  진행도(armFx·swingK·flinch)는 stepStroll이 셈해 넘긴다: 그리는 쪽 시계(rAF)와 움직임의
...
- **방이 서는 자리는 칸이 정한다 — 창 크기가 아니다**(`render.mjs`의 `GRID_COLS`·`assignSlots`).
  전에는 창 폭에 맞춰 방을 좌→우로 흘려 줄바꿈했는데, 창을 줄이면 줄 수가 바뀌는 것으로 끝나지
  않았다 — 한 줄에 앉는 자리 수(그때의 `fitCols`)까지 창 폭을 타서 **방 자체의 폭과 높이가
  달라졌다.** 창을 조금 줄인 것만으로 사무실이 통째로 다시 짜였다.
  지금은 방마다 칸 하나(`box.slot`)를 갖고 그 자리는 설정에 남는다(`view.roomSlots`) —
  **`layout()`은 창 폭을 인자로 받지 않는다.** 열 수를 고정값으로 두는 이유도 같다: 방 개수로
  정하면 세션이 하나 뜰 때마다 열이 늘어 배치가 다시 짜인다.
  손으로 옮기는 것은 **방 이름 띠를 끄는 것**이고(`pickRoomAt` — 자리도 게도 없는 줄이라
  자리 선택과 겹치지 않는다), 놓을 칸은 `cellAt`이 짚는다. **끄는 동안에만 격자를 오른쪽·아래로
  한 칸 펼친다**(`layout`의 `spread`) — 안 펼치면 새 열·새 줄에 놓을 자리가 없다.
  뒤쪽의 빈 열·행은 자르지만 **가운데 빈 칸은 남긴다** — 접어 버리면 화면에 보이는 격자와 놓을
  칸을 짚는 격자가 어긋나 커서 밑의 칸이 실제와 달라진다.
  놓을 때 **화면에 있는 모든 방의 칸을 같이 굳힌다**(`moveRoomTo`) — 옮긴 방 하나만 적어 두면
  자동으로 채워지던 나머지가 빈 칸을 앞에서부터 다시 메우며 같이 움직인다.
  자리를 맞바꾸는 것은 밀어내기가 아니다 — 밀어내면 옆방이 연쇄로 딸려 어디로 갈지 예측할 수 없다.
  판정은 `test/grid.test.mjs`가 훑는다(순수 함수라 node로 돈다)
- `renderer/app.mjs`의 배율·이동 — 손으로 정하는 것은 `SCALES`·`zoomScale`(Ctrl+휠).
  **반 칸 단위이고 2배가 아래 끝이다** — 글자는 확대 밖에서 12px로 그리므로 1배까지 줄이면
  방보다 글자가 커져 이름표가 서로 덮는다(반 칸에서도 픽셀은 또렷하다 — 축을 맞춘 `fillRect`는
  기기 픽셀로 스냅된다). `zoomTo`가 커서 밑 좌표를 붙잡는다.
  자동 배율은 **사무실이 무대에 들어가는 가장 큰 칸**이다(`fitScale`) — 배치가 창을 안 따르게
  된 뒤로 창을 줄였을 때 사무실을 화면에 남겨 주는 일은 배율밖에 할 수 없다. **그래서
  `relayout`은 배치를 먼저 재고 배율을 나중에 정한다**(순서가 뒤바뀌면 배율이 옛 사무실 크기를
  본다). 자동의 아래 끝은 2배가 아니라 **3배**다 — 2배에서는 회의실에서 마주 앉은 두 줄의
  이름표가 겹쳐 읽히지 않는다(굽어서 확인했다). 손으로는 2배까지 내려간다.
  `pickScale`(창 폭 구간)은 이제 미니 전용이다 — 미니는 사무실을 창 크기에 맞춰 다시 세운다.
  다 들어갈 때만 가운데로 다시 놓는 것은 `recenterIfFits`이고 **무대 크기가 바뀌는 순간에만**
  부른다(창 크기·양쪽 열 접기) — 스냅샷마다 부르면 끌어 둔 자리가 그때마다 되돌려지고,
  넘칠 때 가운데로 몰면 좌우가 반씩 잘려 어느 방도 온전히 안 보인다
...
- **미니 창은 `render.mjs`의 다른 함수를 탄다** — `layoutMini`·`renderMini`이고, `app.mjs`에서
  ...
  **미니에서는 전원 서 있다**(`clawdMini`) — 걷기만 끄고 앉히면 `isSeated`가 waiting을 앉음으로
  ...
  `방 · 이름 · 컨텍스트%`를 적는다(`setMiniHover`) — 좁은 창에 말풍선을 띄우면 그게 곧 옆 게를
  덮는다. 컨텍스트에 **라벨을 붙이지 않는다**: `컨텍스트` 네 글자가 220px 줄의 절반을 먹어
  방 이름이 `w…`로 잘렸다. 캔버스 막대와 같은 문턱 색(`level`)으로 대신 낸다 — 오른쪽 계정
  사용률(`#mini-usage`)은 회색이라 색만으로 세션 값과 계정 값이 갈린다
  ([계정 값과 세션 값을 섞지 않는다](panel.md#계정-값과-세션-값을-섞지-않는다)).
  **값이 아니라 키만 들고 있는다** — 담아 두면 스냅샷이 안 오는 동안 멈춘다
  (상단바의 `shownWaitMin`과 같은 함정이라 1초 타이머가 호버 줄도 같이 갱신한다)
- **미니의 계정 사용률(`drawMiniUsage`)은 대기 숫자를 자르지 않는 만큼만 보인다.** 잘린
  `❗2 대…`는 이 창의 존재 이유를 지운다. 판정에서 두 번 틀렸다 — 상자의 넘침만 보면 안 되고
  (방·이름은 줄어들 수 있는 플렉스 아이템이라 상자를 넘치는 대신 **자기가 줄어들며** 안에서
  `…`로 잘린다), 판정은 늘 **"보이는 상태"에서 재고 시작해야** 한다(접은 뒤에 다시 재면 자리가
  남아 보여 켜고, 켜면 또 잘려 스냅샷마다 깜빡인다)
...
`__stroll.tuning()`·`__stroll.selected()`·`__stroll.inBox()`,
그리고 **`__stroll.freeze(true)`**. 마지막 것은 시간을 멈춰 그리기만
...
- **DOM 레이어만 낡은 프레임으로 남는다.** 캔버스는 별도 합성 레이어라 새로 그려지는데,
  ...
  (0.4.0 캡처의 `office.png`가 상단바와 목록이 잘리고 오른쪽 패널이 통째로 빠진 채 나왔다).
  캔버스가 멀쩡하니 스냅샷은 의심되지 않아 찾는 데 오래 걸린다 — 캡처 전에
  `#topbar`의 폭이 `window.innerWidth`와 같은지 **재서 확인한다**
- **한 순간만 훑으면 게가 겹쳐 선 프레임만 잡힌다.** 걷기 구간은 5.2초이고 **세 구간마다 한 번
  다 같이 러그로 모인다**(`HANG_EVERY`). 말풍선 주기(8초)에 맞춰 15초쯤만 훑으면 모인 순간만
  후보에 올라 방 하나에 게가 한 덩어리로 찍힌다. 40~50초를 훑고 **표본 간격을 띄워** 후보를
  고른다(이웃 프레임은 점수가 거의 같아 한 순간이 상위권을 독차지한다)
- **가짜 preload는 창을 만들 때마다 다시 돈다.** `process.env`를 창 사이에 갈아 끼우면
  언어별 픽스처를 따로 물릴 수 있다 — 안 그러면 영어 화면에 한국어 출근부가 박힌다
