...
// 지휘 중의 포인터(산책 모드). **OS 커서를 감추고 이것을 그린다** — 화살표 그대로면 지금
// 지휘 중인지가 화면에 안 드러나고, CSS `crosshair`는 사람마다 모양이 다르다.
//
// 모양은 고전 화살표 그대로 두고 **테두리만 민트로** 바꿨다. 십자 조준선도 그려 봤는데 이
// 크기에서 가는 선 넷은 어설퍼 보였고(눈으로 확인했다), 화살표는 누구나 아는 모양이라
// 위화감이 없다. 색은 명령 표식과 같은 민트다 — 지휘라는 한 벌로 읽힌다.
// 끝점(가리키는 지점)은 **왼쪽 위 첫 픽셀**이다.
//
// 크기는 **10×13이다.** 11×15로 그렸더니 화면에서 OS 커서보다 눈에 띄게 커 보였고, 8×10까지
// 줄이면 이번엔 꼬리가 뭉개져 화살표가 아니라 삼각형이 된다(셋을 나란히 굽어 골랐다).
//
// 9×12로 한 뼘 더 줄여 봤다가 되돌렸다. 고전 화살표는 **밑변이 시작되는 칸이 곧 꼬리의 오른쪽
// 테두리**이고, 그 왼쪽으로 꼬리 두 칸·왼발 두 칸이 들어가야 한다 — 폭이 9면 그 다섯 칸이
// 안 나와서 왼발이 한 픽셀로 쪼그라들거나 머리가 도로 잘록해진다. 그렇게 그린 것이 화면에서는
// 화살표 옆구리에 붙은 지느러미로 보였다(굽어서 확인했다).
